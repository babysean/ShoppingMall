	...
	<div class="paging">
		<ul class="pagination">
			<ul class="pager" id="pageUl">
				<li><a href="bagList.do?cmd=bagList&page=1"
					aria-label="Previous"> <span aria-hidden="true">&laquo;</span></a>
				</li>
				<li><c:if test="${pageBean.currentBlock gt 1}">
						<c:choose>
					...
					</c:if> <c:if test="${pageBean.currentBlock le 1}">
						<c:choose>
							<c:when test="${pageBean.currentPage ne pageBean.startPage}">
				...
				<li><c:if test="${pageBean.totalPage gt pageBean.endPage}">
						<c:choose>
							<c:when test="${pageBean.currentPage ne pageBean.endPage}">
								<a
									href="bagList.do?cmd=bagList&page=${pageBean.currentPage + 1}">Next</a>
							</c:when>
							<c:otherwise>
								<a href="#">Next</a>
							</c:otherwise>
						</c:choose>
					</c:if> <c:if test="${pageBean.totalPage le pageBean.endPage}">
						<c:choose>
							<c:when test="${pageBean.currentPage ne pageBean.endPage}">
								<a
									href="bagList.do?cmd=bagList&page=${pageBean.currentPage + 1}">Next</a>
